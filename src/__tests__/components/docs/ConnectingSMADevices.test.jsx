...
jest.mock('../../../assets/docs/connecting_sma/editingFTP.jpg', () => 'mock-editingFTP.jpg');
jest.mock('../../../assets/docs/connecting_sma/FTPPage.jpg', () => 'mock-FTPPage.jpg');
jest.mock('../../../assets/docs/connecting_sma/location.jpg', () => 'mock-location.jpg');
...
describe('ConnectingSMADevices Component', () => {
  ...
  it('applies correct CSS classes to main heading', () => {
    ...
    const heading = container.querySelector('.mb-8.flex.text-2xl.font-bold.sm\\:text-4xl');
  ...
  describe('Step Components', () => {
    ...
    it('renders step 2 with cluster controller navigation instructions', () => {
      ...
      expect(step2).toHaveAttribute('data-title', 'Locate the settings page on your SMA cluster controller');
      
      expect(screen.getByText(/Login to your cluster controller and navigate to the/)).toBeInTheDocument();
    ...
    it('renders step 3 with FTP setup instructions', () => {
      ...
      expect(step3).toHaveAttribute('data-title', 'Setting up FTP push data transfer');
      
      expect(screen.getByText(/There are a few fields to update:/)).toBeInTheDocument();
    });
    ...
    it('renders step 4 with device validation instructions', () => {
      ...
      expect(step4).toHaveAttribute('data-title', 'Validate device was created');
      
      expect(screen.getByText(/Once your SMA cluster controller has gone through a push data cycle/)).toBeInTheDocument();
    });
    ...
    it('renders step 5 with location setup instructions', () => {
      ...
      expect(screen.getByText(/It is important to properly set the site's location/)).toBeInTheDocument();
    });
  });
  ...
  describe('Step 2 Content', () => {
    it('applies brand styling to navigation elements', () => {
      ...
      expect(furtherApplications).toHaveClass('font-bold', 'text-brand-primary');
    });
  ...
  describe('Step 3 Content', () => {
    it('renders CSV format instruction with brand styling', () => {
      ...
      expect(screen.getByText(/Data export in CSV format should be set to/)).toBeInTheDocument();
    ...
    it('renders XML format instruction with brand styling', () => {
      ...
      expect(screen.getByText(/Data export in XML format should be set to/)).toBeInTheDocument();
    ...
    it('renders login and password instruction', () => {
      ...
      expect(screen.getByText(/Login & password fields should be updated with your access key retrieved in step 1/)).toBeInTheDocument();
    });

    it('renders server URL with copy button', () => {
      render(<ConnectingSMADevices />);
      
      expect(screen.getByText('ftp.solarmoonanalytics.com')).toBeInTheDocument();
      const copyButton = screen.getByTestId('copy-button');
      expect(copyButton).toBeInTheDocument();
      expect(copyButton).toHaveAttribute('data-datasrc', 'ftp.solarmoonanalytics.com');
    ...
    it('renders editing FTP image with correct attributes', () => {
      ...
      expect(editingFtpImage).toHaveClass('object-fill', 'py-6', 'pe-6', 'ps-3');
    });
    ...
    it('uses flexbox layout for server field', () => {
      ...
      const serverField = screen.getByText(/Server should be updated to/).closest('div');
  ...
  describe('Step 4 Content', () => {
    ...
    it('renders site management page link', () => {
      ...
      expect(managementLink).toHaveAttribute('href', 'https://app.solarmoonanalytics.com/manage');
    ...
    it('explains device naming convention', () => {
      ...
      expect(screen.getByText(/Devices will show up under a site named by the cluster controller's name/)).toBeInTheDocument();
    });
    ...
    it('renders support email link', () => {
      ...
      expect(supportLink).toHaveAttribute('href', 'mailto:support@solarmoonanalytics.com');
    ...
    it('includes troubleshooting guidance', () => {
      ...
      expect(screen.getByText(/validate within the SMA controller's event logs/)).toBeInTheDocument();
    });
  });
  ...
  describe('Step 5 Content', () => {
    it('explains importance of location setting', () => {
      ...
      expect(screen.getByText(/It is important to properly set the site's location/)).toBeInTheDocument();
      expect(screen.getByText(/guarantee all collected data is properly stamped with the time zone specific time/)).toBeInTheDocument();
    });

    it('mentions fallback to user profile timezone', () => {
      render(<ConnectingSMADevices />);
      
      expect(screen.getByText(/the platform will fall back to the default timezone set on the user profile/)).toBeInTheDocument();
    });
  ...
  describe('Layout Structure', () => {
    it('maintains proper component hierarchy', () => {
      ...
      const headingDiv = mainDiv.querySelector('.mb-8.flex.text-2xl.font-bold.sm\\:text-4xl');
  ...
  describe('Accessibility', () => {
    ...
    it('maintains proper text hierarchy with brand styling', () => {
      ...
      expect(heading.closest('div')).toHaveClass('text-2xl', 'font-bold', 'sm:text-4xl');
      
      // Brand elements should have consistent styling
      const brandElements = screen.getAllByText((content, element) => 
  ...
  describe('SMA-Specific Features', () => {
    it('covers SMA cluster controller configuration', () => {
      ...
      expect(screen.getAllByText(/cluster controller/).length).toBeGreaterThan(0);
      expect(screen.getByText('Settings tab.')).toBeInTheDocument();
      expect(screen.getByText('Further Applications')).toBeInTheDocument();
    });
    ...
    it('addresses timezone and location concerns', () => {
      ...
      expect(screen.getByText(/default timezone set on the user profile/)).toBeInTheDocument();
    });
  });
  ...
  describe('User Experience', () => {
    ...
    it('provides troubleshooting guidance', () => {
      ...
      expect(screen.getByText(/If no devices shows up, validate within the SMA controller's event logs/)).toBeInTheDocument();
      expect(screen.getByText(/If a further error occurs, please contact/)).toBeInTheDocument();
    });
  });
});