...
describe('Step Component', () => {
  ...
  it('renders step with number, title and content', () => {
    ...
    expect(screen.getByText('This is test content for the step component')).toBeInTheDocument();
  });
  ...
  it('renders with React element as content', () => {
    const elementContent = <div data-testid="custom-content">Custom React Element</div>;
    render(<Step number={2} title="Element Test" content={elementContent} />);
  ...
  it('handles different number values', () => {
    render(<Step number={99} title="High Number" content="Test content" />);
    
    expect(screen.getByText('99')).toBeInTheDocument();
  });

  it('applies spacer when no number provided', () => {
    const { container } = render(<Step number={null} title="No Number" content="Test" />);
