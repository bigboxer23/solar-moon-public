...
jest.mock('../../../assets/upload-channel.jpg', () => 'mock-upload-channel.jpg');
...
describe('ConnectingDevice Component', () => {
  ...
  describe('Step Components', () => {
    ...
    it('renders step 2 with correct title and login instructions', () => {
      ...
      expect(step2).toHaveAttribute('data-title', 'Login to your Obvius data acquisition server');
      
      expect(screen.getByText(/Login to your device and navigate to the/)).toBeInTheDocument();
      expect(screen.getByText('Log File Data -> Setup/Upload')).toBeInTheDocument();
    });
    ...
    it('renders step 3 with upload channel setup instructions', () => {
      ...
      expect(screen.getByText(/There are a few things to change in this step:/)).toBeInTheDocument();
    });
    ...
    it('renders step 4 with data upload trigger instructions', () => {
      ...
      expect(step4).toHaveAttribute('data-title', 'Trigger sending data from the server to the platform');
    ...
    it('renders step 5 with device validation instructions', () => {
      ...
      expect(step5).toHaveAttribute('data-title', 'Validate device was created');
  ...
  describe('Step 3 Content', () => {
    ...
    it('renders upload URL with copy button', () => {
      ...
      expect(screen.getByText('http://solarmoonanalytics.com/upload')).toBeInTheDocument();
      const copyButton = screen.getByTestId('copy-button');
      expect(copyButton).toBeInTheDocument();
      expect(copyButton).toHaveAttribute('data-datasrc', 'http://solarmoonanalytics.com/upload');
    ...
    it('applies correct styling to upload URL', () => {
      ...
      const uploadUrl = screen.getByText('http://solarmoonanalytics.com/upload');
      expect(uploadUrl).toHaveClass('me-2');
      
      const urlContainer = uploadUrl.closest('div');
      expect(urlContainer).toHaveClass('flex', 'flex-wrap', 'items-center', 'font-bold', 'text-brand-primary');
    });

    it('renders password update instruction', () => {
      render(<ConnectingDevice />);
      
      expect(screen.getByText(/Password should be updated with your Access Key retrieved in step 3/)).toBeInTheDocument();
    });

    it('renders filter setup instruction', () => {
      render(<ConnectingDevice />);
      
      expect(screen.getByText(/Optionally set filters to determine which devices are sent to the platform/)).toBeInTheDocument();
    });
  ...
  describe('Step 5 Content', () => {
    ...
    it('includes device validation instructions', () => {
      ...
      expect(screen.getByText(/validate you see your device/)).toBeInTheDocument();
      expect(screen.getByText(/If no device shows up, validate within the Obvius device's logs/)).toBeInTheDocument();
    });
  });
  ...
  describe('Responsive Design', () => {
    ...
    it('uses responsive layout classes for URL container', () => {
      ...
      const urlContainer = screen.getByText('http://solarmoonanalytics.com/upload').closest('div');
